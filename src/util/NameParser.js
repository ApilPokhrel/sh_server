var NameParse = (function() {
  ...
  NameParse.parse = function(fullast) {
    fullast = fullast.trim();
    ...
    nameParts = fullast.split(" ").filter(function(namePart) {
    ...
    if (end - start > 1) {
      ...
      for (j = i; j < end; j++) {
        last += " " + this.fix_case(nameParts[j]);
      }
    }
  ...
  NameParse.is_compound_last = function(word) {
